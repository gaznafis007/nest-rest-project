...
export class PostController {
    ...
    @Get(':id')
    findById(@Param('id', ParseIntPipe, PostExistPipe) id: number) : IPost {
    ...
    updatePost(@Param('id', ParseIntPipe, PostExistPipe) id: number, @Body() post: Partial<Omit<IPost, 'id' | 'createdAt' | 'updatedAt'>>): IPost {
        return this.postService.updatePost(id, post);
    };

    @Delete(':id')
    @HttpCode(HttpStatus.OK)
    deletePost(@Param('id', ParseIntPipe, PostExistPipe) id: string): {message: string}{
